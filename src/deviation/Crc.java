...
public class Crc {
    ...
    static public long Crc32(byte [] data)
    {
        return Crc.Crc32(data, 0xffffffffL);
    }
    static public long Init32() {
        return 0xffffffffL;
    }
    //This routine comes from 'cliv1' on the stm32 forums.
    //No link because the forums keep changing the path, but the thread title is:
    //CRC computation
    static public long Dfu32(long [] data, long crc)
    {
        for (long d : data) {
            crc = crc ^ d;
            ...
            for (int i=0; i<32; i++) {
                if ((crc & 0x80000000L) != 0) {
                    crc = (0xFFFFFFFFL & (crc << 1)) ^ 0x04C11DB7L; // Polynomial used in STM32
                } else {
